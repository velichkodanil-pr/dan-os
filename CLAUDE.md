...
- Domains `personal` / `travelon` / `tech` are real isolation boundaries, not just columns (R6.1B). The active domain is decided server-side at request start and passed down as an immutable snapshot — never from model output, tool args, callback data or client JSON. `domain` is a mandatory argument on every core function (no production default of "personal"); it is not in any tool's input schema, so the model cannot choose it. RAG, wiki, memory, chat, tasks, Google accounts and background jobs are all domain-scoped with no cross-domain fallback. No `domain="all"`; no LLM prompt ever mixes domains; the only cross-domain ops are deterministic, owner-only and explicitly labelled (security scan, `/accounts`, `/domain_audit`, scheduled rituals built from separate per-domain queries). Domain is never guessed from content — legacy backfill inherits from a trusted parent only.
