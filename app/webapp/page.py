...
async function act(action, id, version) {
...
function renderToday() {
  ...
  if (d.goals.length) {
    h += '<h2>🎯 Цілі</h2>' + d.goals.map(g => `
      <div class="card row"><div class="grow title">${esc(g.title)}</div>
      <button class="ok" onclick="act('goal_done','${g.id}')">🏁</button></div>`).join('');
  }
  if (d.travelon) h += '<div class="card sub">🧳 TravelON пульс — команда /travelon у чаті</div>';
  return h || '<div class="empty">Порожньо — гарного дня ✨</div>';
}
...
function render() {
  ...
  view.innerHTML = TAB === 'today' ? renderToday()
                 : TAB === 'review' ? renderReview() : renderMemory();
